...
export default function HealthAnalysisPage() {
  ...
  return (
    <div className="flex-grow w-full px-4 md:px-10 lg:px-16 py-6 md:py-10 grid grid-cols-1 lg:grid-cols-12 gap-6 relative">
      ...
      {/* Main Analysis Panel */}
      <section className="lg:col-span-8 flex flex-col min-h-[600px] lg:h-[calc(100vh-140px)] glass-panel rounded-[2rem] p-5 md:p-8 shadow-xs">
        {/* Header */}
        <div className="mb-4 pb-4 border-b border-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shrink-0">
          <div>
            <h1 className="text-[24px] md:text-[28px] leading-tight font-extrabold text-slate-900 tracking-tight">Symptom Assessment</h1>
          ...
          <div className="flex items-center gap-2.5 bg-teal-50 px-3.5 py-1.5 rounded-full border border-teal-200 shrink-0">
            <span className="relative flex h-2.5 w-2.5">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-500 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-teal-600"></span>
            </span>
            <span className="text-[12px] font-bold text-teal-800 uppercase tracking-wider">AI Active</span>
          </div>
        </div>

        {/* Quick Symptoms Chips */}
        <div className="mb-3 flex flex-wrap gap-2 shrink-0 items-center">
          ...
          {quickSymptoms.map(s => (
            <button 
              ...
              className="px-3 py-1 rounded-xl border border-slate-200 bg-white text-slate-700 text-[13px] font-semibold hover:bg-teal-600 hover:text-white hover:border-teal-600 transition-all active:scale-95 shadow-2xs"
            >
              + {s}
            </button>
          ))}
